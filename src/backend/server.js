...
const handleQuery = (url, instance) => {
  app.get(url, async (req, res) => {
    ...
    if (instance.getError() !== null) {
      res.status(500).json({
        // Changed to handle other additonal sever errors
      ...
      console.log(
        `Server: ${
          instance.getPayload().length
        } posts successfully sent to client.`
      );
    }
  });
};
...
// MongoDB connection
mongoose
  .connect("mongodb://localhost:27017/Jumpstart", {
...
app.post("/api/register", async (req, res) => {
  ...
  if (!email || !email.endsWith("@unomaha.edu")) {
    return res
      .status(400)
      .json({ message: "Please provide a valid @unomaha.edu email address." });
  }
...
app.post("/api/verify-code", async (req, res) => {
  ...
  if (1) {
    // This is me skipping writing to DB rn
